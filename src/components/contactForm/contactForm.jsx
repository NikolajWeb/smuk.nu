...
const ContactForm = () => {
  ...
  const [memberData, setMemberData] = useState(null); // Gemmer data der skal vises i success beskeden
  ...
  const onSubmit = async (data) => {
    try {
      localStorage.setItem("memberData", JSON.stringify(data)); // Gemmer data i localStorage
      setMemberData(data); // Sætter state så vi kan vise success besked
      setSuccess(true); // Viser succes-view
  ...
  // Viser success besked når formularen er udfyldt korekt
  ...
  // Viser selve formularen hvis der ikke er sendt data
  return (
    <form className={styles.form} onSubmit={handleSubmit(onSubmit)}>
      ...
      {/* Submit-knap – tekst skifter når formularen submitter */}
      <Button
        className={styles.btn}
        buttonText={isSubmitting ? "Sender..." : "Opret"}
      />
    </form>
  );
};
